...
class Laratube 
{
    ...
    {
        return DB::table('laratube_access_tokens')->insert([
    ...
    {
        $latest = DB::table('laratube_access_tokens')
            ->latest('created_at')
            ->first();
    ...
    {
        if(
            !$this->app->config->get('laratube.client_id') ||
            !$this->app->config->get('laratube.client_secret')
        ) {
            throw new Exception('A Google "client_id" and "client_secret" must be configured.');
        }
        $client->setClientId($this->app->config->get('laratube.client_id'));
        $client->setClientSecret($this->app->config->get('laratube.client_secret'));
        $client->setScopes($this->app->config->get('laratube.scopes'));
        $client->setAccessType('offline');
        $client->setApprovalPrompt('force');
        $client->setRedirectUri(url(
            $this->app->config->get('laratube.routes.prefix')
            . '/' .
            $this->app->config->get('laratube.routes.redirect_uri')
        ));
